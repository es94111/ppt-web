...
export function CollaboratorManager({ deckId }: { deckId: string }) {
  ...
  return <div className="manager-stack">
    ...
    <div className="manager-list">{items.length ? items.map((item) => <article className="manager-item" key={item.id}><div><strong>{item.user.name || item.user.email}</strong><p className="muted">{item.user.email}</p></div><div className="manager-actions"><select className="input compact-input" value={item.role} onChange={(e) => update(item.id, e.target.value as Collaborator["role"])}><option value="VIEWER">可觀看</option><option value="COMMENTER">可留言</option><option value="EDITOR">可編輯</option></select><button className="icon-btn" onClick={() => remove(item.id)}><X size={16} /></button></div></article>) : <div className="empty compact">尚未加入協作者</div>}</div>
  </div>;
}
