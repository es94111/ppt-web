import { cookies } from "next/headers";
import { notFound, redirect } from "next/navigation";
import { auth } from "@/auth";
import { db } from "@/lib/db";
import { verifyDeckAccessToken } from "@/lib/http";
import { brandKitFromDeck } from "@/lib/brand";
import { DeckPassword } from "@/components/DeckPassword";
import { Viewer } from "@/components/Viewer";
import "@/app/viewer.css";

export const dynamic = "force-dynamic";
export default async function ViewPage({ params }: { params: Promise<{ id: string }> }) {
  const session = await auth();
  const { id } = await params;
  const deck = await db.deck.findUnique({ where: { id }, include: { slides: { orderBy: { order: "asc" } }, collaborators: { where: { userId: session?.user.id ?? "__anonymous__" }, select: { role: true } } } });
  if (!deck) notFound();
  const owns = !!session?.user && (session.user.role === "ADMIN" || deck.ownerId === session.user.id);
  const collaborates = !!session?.user && deck.collaborators.length > 0;
  // 私人：須登入且為擁有者/Admin
  if (deck.visibility === "PRIVATE" && !owns && !collaborates) redirect(session?.user ? "/dashboard" : "/login");
  if (deck.visibility === "AUTHENTICATED" && !session?.user) redirect(`/login?callbackUrl=${encodeURIComponent(`/d/${id}`)}`);
  // 公開簡報可額外設定密碼；擁有者與管理員可直接觀看。
  const accessToken = (await cookies()).get(`deck_access_${id}`)?.value;
  if (deck.passwordHash && !owns && !verifyDeckAccessToken(id, accessToken)) return <main><DeckPassword deckId={id} title={deck.title} /></main>;
  // PUBLIC / UNLISTED 允許匿名；AUTHENTICATED 已在上方確認登入狀態。
  if (deck.status !== "READY") {
    const text = deck.status === "PROCESSING" ? "PPTX 轉檔中，請稍後重新整理。" : "PPTX 轉檔失敗，請重新上傳。";
    return <main><section className="container section"><div className="empty"><h3>{deck.title}</h3><p>{text}</p></div></section></main>;
  }
  return <main><Viewer deckId={id} title={deck.title} exitHref={session?.user ? "/dashboard" : "/"} pptxUrl={deck.sourceType === "PPTX" && deck.sourceFile ? `/api/decks/${id}/pptx` : undefined} brandKit={brandKitFromDeck(deck)} slides={deck.slides.map((s) => ({ id: s.id, order: s.order, content: s.content, notes: s.notes }))} /></main>;
}
