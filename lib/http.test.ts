...
describe("deck access token",()=>{it("is scoped and tamper resistant",()=>{const token=createDeckAccessToken("deck-a",60);expect(verifyDeckAccessToken("deck-a",token)).toBe(true);expect(verifyDeckAccessToken("deck-b",token)).toBe(false);expect(verifyDeckAccessToken("deck-a",`${token}x`)).toBe(false);expect(verifyDeckAccessToken("deck-a",`${token}.extra`)).toBe(false)})});
